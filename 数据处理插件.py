...
def get_deliver(sales_file, stock_file, on_the_way=6, mini_security_day=7, security_day=30):
    ...
    df_last.to_excel(stock_file.replace('.xlsx', aft))
...
def get_daily(data_path):
    ...
    # data_sum['asin'] = data_sum['(父） ASIN']
    # data_sum['sub_asin'] = data_sum['（子）ASIN']
...
def main():
    ...
    print('请选择要使用的功能:')
    print('-'*25)
    for i in ['1: 表格合并', '2: 添加图片辅助列', '3: 生成发货表格']:
        print('-'*8 + '{:-<20}'.format(i))
    print('-'*25)

    while True:
        try:
            mode = int(input('输入1-3对应功能的数字：'))
            if mode in [1, 2, 3]:
                break
        except Exception as e:
            print(e)
...
if __name__ == '__main__':
    # main()
    # file_path = 'data_info2019-0418.xlsx'
    # data_path = r'D:\Users\Administrator\Phe\陆\HA-子商品数据\HA-子商品数据'
    data_path = 'D:\待合并'
    in_all(data_path, fill_name=False)
